...
{	
	...
	ip netns exec $NS_NAME ip route add default via 192.168.0.10
...
then
	...
	echo "Adding ip 192.168.0.10/24 to bridge br0"
	ip addr add 192.168.0.10/24 dev br0
	echo "-----------------------------------------------"
	create_namespace "cont1" 192.168.0.1/24 192.168.0.2/24
	echo "-----------------------------------------------"
	create_namespace "cont2" 192.168.0.3/24 192.168.0.4/24
	echo "-----------------------------------------------"
	echo "Setting bridge up"
	ip link set dev br0 up
	echo "Setting route to another node"
	ip route add 192.168.1.0/24 via 10.0.0.12 dev enp0s8
	echo "Enabling IP forwarding"
	sysctl -w net.ipv4.ip_forward=1
	echo "Deleting some default route"
	sudo ip route del 192.168.0.0/24 dev veth-cont1
	sudo ip route del 192.168.0.0/24 dev veth-cont2
	exit 0
else
	...
	ip link set dev br0 down
	brctl delbr br0
	ip route del 192.168.1.0/24 via 10.0.0.12 dev enp0s8
